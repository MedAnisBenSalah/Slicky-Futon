package com.ormisiclapps.slickyfuton.input;
...
class InputProcessorHandler implements InputProcessor
{
    ...
    public boolean touchDown(int x, int y, int pointer, int button)
    {
        ...
        // Report the touch
        Core.getInstance().getInputHandler().registerInputEvent(ScreenTouchType.SCREEN_TOUCH_DOWN, x, y);
        return false;
    }
    ...
    {
        // Report the touch
        Core.getInstance().getInputHandler().registerInputEvent(ScreenTouchType.SCREEN_TOUCH_UP, x, y);
        return false;
    }
    ...
    public boolean touchDragged(int x, int y, int pointer)
    {
        ...
        if(downPosition.x != x || downPosition.y != y)
            Core.getInstance().getInputHandler().registerInputEvent(ScreenTouchType.SCREEN_TOUCH_DRAGGED, x, y);
        else
        {
            // Record the down position
            downPosition.set(x, y);
    ...
    {
    	Core.getInstance().getInputHandler().registerMouseMovement(x, y);
        return false;
    }
